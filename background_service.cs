...
class Program
{
    ...
    static void ExtractArt(string dir, GlobalSystemMediaTransportControlsSessionMediaProperties props)
    {
        ...
        try
        {
            ...
            if (artOp.Status != Windows.Foundation.AsyncStatus.Completed) return;
    ...
    static void Main(string[] args)
    {
        ...
        string smtcFile = Path.Combine(dir, "smtc_data.json");
        string lastJson = "";
        string lastArtTitle = "";
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (!ready.WaitOne(5000) || mgr == null) { ready.Dispose(); Thread.Sleep(5000); continue; }
                ready.Dispose();
                ...
                while (true)
                {
                    try
                    {
                        ...
                        if (props != null && !string.IsNullOrEmpty(props.Title))
                        {
                            ...
                            if (props.Title != lastArtTitle)
                            {
                                lastArtTitle = props.Title;
                                ExtractArt(dir, props);
                            }
                        }
                    ...
                    write:
                        ...
                        {
                            lastJson = json;
                            try { File.WriteAllText(smtcFile, json); } catch { }
                        }
                    }
                    catch { json = "{\"status\":\"error\"}"; if (json != lastJson) { lastJson = json; try { File.WriteAllText(smtcFile, json); } catch { } } }
                    Thread.Sleep(1000);
                }
            }
            catch
            {
                try { File.WriteAllText(smtcFile, "{\"status\":\"error\"}"); } catch { }
                Thread.Sleep(5000);
            }
        }
    }
